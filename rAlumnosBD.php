...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $boleta = isset($_POST['boleta']) ? htmlspecialchars($_POST['boleta']) : '';
    $correo = isset($_POST['correo']) ? htmlspecialchars($_POST['correo']) : '';
    $password = isset($_POST['password']) ? $_POST['password'] : '';

    // Verifica que todos los campos requeridos estén presentes
    if (empty($firstName) || empty($lastName) || empty($boleta) || empty($correo) || empty($password)) {
    ...
    $sql = "INSERT INTO alumnos (Boleta, Nombre, Apellidos, Correo, Password)
            VALUES (:boleta, :firstName, :lastName, :correo, :password)";
